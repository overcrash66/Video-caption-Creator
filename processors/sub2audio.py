...
class SubToAudio:
  ...
  def __init__( self,model_name:str=None,model_path:str=None,config_path:str=None,progress_bar:bool=False,fairseq_language:str=None,**kwargs,):
  ...
  def coqui_model(self) -> list:
      """Return list of available model names using CLASS METHOD"""
      try:
          return TTS.list_models()  # Correct class method call
      except Exception as e:
          print(f"Error listing models: {e}")
          return []

  def _model_exists(self, model_name: str) -> bool:
      """Check for XTTS v2 model files"""
      try:
          model_path = os.path.expandvars(r"%LOCALAPPDATA%\tts\tts_models--multilingual--multi-dataset--xtts_v2")
          required_files = {
              'model.pth',
              'config.json',
              'vocab.json',
              'speakers_xtts.pth'  # XTTS v2 specific file
          }
          existing_files = set(os.listdir(model_path))
          return required_files.issubset(existing_files)
      except Exception as e:
          print(f"XTTS v2 check failed: {e}")
          return False

  def coqui_model(self) -> list:
      """Return available models including XTTS v2"""
      return [
